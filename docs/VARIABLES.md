...
> **Required variables are marked with \*** — all others are optional.
...
| `{{GENERATION_DATE}}` | When generated | "2026-03-29" |
...
|----------|-------------|--------|
| `{{SECURITY_WARNINGS_ENABLED}}` * | Show security warnings | true/false (default: true) |
| `{{CODE_EXECUTION_WARNINGS}}` * | Warn before executing code | true/false (default: true) |
| `{{AUTO_SEND_ENABLED}}` | Allow auto-send messages | true/false (default: false) |
| `{{REQUIRE_CONFIRMATION}}` | Confirm before actions | true/false (default: true) |
| `{{SENSITIVE_DATA_HANDLING}}` | How to handle private data | "Never store/Always encrypt/Ask first" |
...
{{#if EMAIL_ENABLED}}
### Email Setup
Configure your Gmail or Outlook credentials here.
{{/if}}
```
